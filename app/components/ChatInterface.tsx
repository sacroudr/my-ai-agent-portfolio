...
//   // Session ID + message persistence
//   useEffect(() => {
...
//     // Restore messages from localStorage for this session
//     try {
//       const stored = localStorage.getItem(`portfolio_messages_${sessionId}`);
//       if (stored) {
//         const parsed = JSON.parse(stored) as Message[];
//         if (Array.isArray(parsed) && parsed.length > 0) {
//           // Mark all messages as not streaming (they're historical)
//           setMessages(parsed.map((m) => ({ ...m, isStreaming: false })));

//           // Restore last detected language
//           const lastAssistant = [...parsed].reverse().find((m) => m.role === "assistant" && m.language);
//           if (lastAssistant?.language) {
//             setConversationLanguage(lastAssistant.language as "fr" | "en");
//           }
//         }
//       }
//     } catch {
//       // localStorage read failed — start fresh
//     }
//   }, []);
...
//         `portfolio_messages_${sessionIdRef.current}`,
...
//           const errorMsg = conversationLanguage === "fr"
//             ? (data.message_fr || "Limite atteinte. Réessayez dans une heure.")
//             : (data.message || "Rate limit reached. Please try again in an hour.");
...
//     // Uses a stable key "portfolio_last_messages" — survives tab/browser close
//     try {
//       const stored = localStorage.getItem("portfolio_last_messages");
//       if (stored) {
...
//           // Restore the old session ID so the conversation continues correctly
...
//             .reverse()
//             .find((m) => m.role === "assistant" && m.language);
//           if (lastAssistant?.language) {
//             setConversationLanguage(lastAssistant.language as "fr" | "en");
//           }
//         }
...
//       };
//       localStorage.setItem("portfolio_last_messages", JSON.stringify(toSave));
//     } catch {
...
//                 sessionStorage.setItem("portfolio_session_id", newId);
//                 sessionIdRef.current = newId;
//                 localStorage.removeItem("portfolio_last_messages");
//                 setMessages([]);
//               }}
//               style={{
//                 display: "flex", alignItems: "center", gap: "0.4rem",
            ...
            {isLoading ? (
              ...
              <>
                ...
                <span style={{
                  ...
                  {conversationLanguage === "fr" ? "Riad's AI réfléchit..." : "Riad's AI is thinking..."}
                </span>
              </>
            ) : input.trim().length > 0 ? (
              // User is typing
              ...
              // Ready state — shown after first message
              <>
                <div style={{
                  width: 5, height: 5, borderRadius: "50%",
