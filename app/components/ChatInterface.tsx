...
//   "Hi, I'm Riad's AI. Ask me anything about his work, stack, or availability — in English or French.";

// // Render queue constants — 3 chars per 20 ms ≈ 150 chars/sec, feels natural
...
//   // Streaming render queue — decouples network speed from visible typing speed
//   const renderQueueRef = useRef<string>("");
//   const renderIntervalRef = useRef<NodeJS.Timeout | null>(null);
//   const streamingMessageIdRef = useRef<string>("");
//   const streamFinishedRef = useRef<boolean>(false);
//   // Stores the last user query so the error retry button can re-send it
...
//     if (lastUserMessage) {
//       const frenchWords = [
//         "je", "tu", "il", "elle", "nous", "vous", "est", "sont",
//         "que", "qui", "quoi", "comment", "pourquoi", "bonjour", "merci",
//         "oui", "non", "avec", "pour", "dans", "sur", "les", "des", "une",
//         "parle", "quel", "quelle", "quels", "peux", "peut", "veux", "mais", "aussi",
//       ];
//       const words = lastUserMessage.content.toLowerCase().split(/\s+/);
//       const frenchCount = words.filter((w) =>
//         frenchWords.includes(w.replace(/[^a-zàâäéèêëîïôùûüç]/g, ""))
//       ).length;
...
//       // Clear any in-flight render state from a previous message
...
//       // Start the render interval — drains the queue at a human-readable pace.
//       // Network tokens land in renderQueueRef; only this interval writes to state.
//       renderIntervalRef.current = setInterval(() => {
//         if (renderQueueRef.current.length === 0) {
//           // Queue is empty: if the stream is also done, we're finished
...
//           return;
//         }
//         const toRender = renderQueueRef.current.slice(0, CHARS_PER_TICK);
//         renderQueueRef.current = renderQueueRef.current.slice(CHARS_PER_TICK);
//         setMessages((prev) =>
//           prev.map((m) =>
//             m.id === streamingMessageIdRef.current
//               ? { ...m, content: m.content + toRender, isStreaming: true }
//               : m
//           )
//         );
//       }, TICK_MS);
...
//                 // Push into queue only — the interval handles rendering
//                 renderQueueRef.current += text;
//               } catch {}
//             }
//           }
//         }

//         // Signal to the interval that no more tokens are coming.
//         // isLoading and isStreaming will be cleared once the queue drains.
//         streamFinishedRef.current = true;
//       } catch (err: any) {
//         // Clean up interval immediately on any error
//         if (renderIntervalRef.current) {
//           clearInterval(renderIntervalRef.current);
//           renderIntervalRef.current = null;
//         }
...
//         {/* without interrupting whatever the user is currently hearing.           */}
//         <div
//           role="log"
//           aria-live="polite"
...
//               }}
//             >
//               {/* Ambient glow — CSS-only radial gradient that slowly pulses */}
//               <div
//                 style={{
//                   position: "absolute",
//                   top: "50%",
...
//                   }}
//                 >
//                   RAG · Claude Haiku · pgvector · Next.js
//                 </p>
//               </div>
...
//                 message.isError
//                   ? () => {
//                       // Remove the error assistant message, then re-send the last query
...
//           {/* Typing indicator — role="status" announces the state to screen readers */}
...
//               >
//                 {isListening ? (
//                   /* #EF4444 is intentionally off-palette — universal "recording" red */
//                   <div
//                     style={{
//                       width: 10,
//                       height: 10,
...
//                 borderRadius: "var(--radius-sm)",
//                 background:
//                   input.trim() && !isLoading ? "var(--accent)" : "var(--bg-subtle)",
//                 border: "none",
...
//                 justifyContent: "center",
//                 color:
//                   input.trim() && !isLoading ? "var(--bg-base)" : "var(--text-muted)",
//                 flexShrink: 0,
//                 transition: "all 0.15s ease",
//               }}
//               onMouseEnter={(e) => {
...
//               }}
//               onMouseDown={(e) => {
//                 if (input.trim() && !isLoading) e.currentTarget.style.transform = "scale(0.97)";
//               }}
//               onMouseUp={(e) => {
//                 e.currentTarget.style.transform = "scale(1)";
//               }}
