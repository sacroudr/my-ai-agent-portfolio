...
// import { buildSystemPrompt, getConfidenceLevel } from "@/lib/rag/systemPrompt";
...
//     const confidence = getConfidenceLevel(relevantChunks);
...
//           // Send confidence level so the frontend can show an indicator
//           controller.enqueue(encoder.encode(`c:${JSON.stringify(confidence)}\n`));

//           // Send detected language so contact form uses correct locale
//           controller.enqueue(encoder.encode(`l:${JSON.stringify(language)}\n`));
...
import { buildSystemPrompt, getConfidenceLevel } from "@/lib/rag/systemPrompt";
import { detectLanguage } from "@/lib/detectLanguage";
import { getOrCreateSession, logMessage, sendNewSessionNotification } from "@/lib/db/helpers";
